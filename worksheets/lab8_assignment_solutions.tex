...
\title{\textbf{MIT 6.390 Introduction to Machine Learning} \\ \LARGE Lab 8: Representation Learning \& Latent Space Evolution \\ \Large \textit{A Deep Dive into Neural Concept Formation} \\ \vspace{0.5cm} \textcolor{red}{\textbf{[TEACHING ASSISTANT ANSWER KEY]}}}
...
To understand modern Deep Learning, we must first understand the ``Representation Problem.'' In early machinelearning paradigms (which we explored in the first half of this course), we explicitly designed hand-crafted features. For example, to classify an image of a dog, a human engineer might write code to detect edges, compute color histograms, and identify shapes like ``ears'' or ``snouts.'' For text classification, we traditionally rely on \textbf{Bag of Words} or \textbf{TF-IDF} (Term Frequency-Inverse Document Frequency) vectors, which count the occurrences of specific words in a vocabulary of size $V$.
...
\begin{enumerate}
    ...
    \item Navigate your web browser to \href{https://sphinx-agent.me/mantis}{Mantis}.
...
\clearpage
\section{Assignment Questions \& Solution Key}
...
\textcolor{blue}{\textbf{Solution:} TF-IDF vectors strictly score exact word matches. Therefore, a massive 10,000+ length dimension is required just to represent simple vocabularies. Furthermore, because TF-IDF uses orthogonal axes for every single word, it completely fails to capture semantic meaning or context—words like ``car'' and ``automobile'' are entirely unlinked in TF-IDF geometry, whereas a neural embedding learns to squash and map both concepts to incredibly close geometric coordinates in dense space.}
\vspace{0.75cm}
...
\textcolor{blue}{\textbf{Solution:} The final softmax or linear classifier layer simply outputs independent class probabilities (e.g. 90\% Art, 10\% Physics) which tells us little to nothing about the underlying conceptual relationship between those classes inside the model's structural geometry. By extracting the penultimate linear transformations, we capture the high-dimensional representation vector representing the raw semantic features that the neural network "sees". This lets us visualize overlapping semantic meaning rather than mutually exclusive rigid prediction classes.}
\vspace{0.75cm}
...
\textcolor{blue}{\textbf{Solution:} As training minimizes the cross-entropy loss, the gradient mathematically pushes data samples belonging to different target categories apart while pulling samples from the same category into tight overlapping centroids. Therefore, the Euclidean gap between ``Quantum Physics'' and the completely unrelated ``Renaissance Art'' points will grow massively. Conversely, vectors for ``Machine Learning'' and ``Pure Mathematics'' may actually end up physically close to each other or forming bordering clusters rather than totally repelling, because their underlying language structures, equations, and vocabularies are heavily correlated mathematically.}
\vspace{0.75cm}
...
\textcolor{blue}{\textbf{Solution:} The UMAP graph typically looks like a chaotic, overlapping, undifferentiated cloud of noise because the initial random PyTorch weights matrix projects the TF-IDF feature space randomly across multiple dimensions. There is generally zero inherent semantic structural clustering, meaning points for completely different categories are uniformly distributed and mixed up throughout the initial vector space.}
\vspace{0.75cm}
...
\textcolor{blue}{\textbf{Solution:} \texttt{.detach()} separates the tensor cleanly from PyTorch's intensive computational and gradient tracking graph. Without it, the \texttt{embeddings\_per\_epoch} array would unknowingly maintain references to the entire backward-propagation history tree generated by PyTorch in every single training loop pass! Over 11 epochs, storing 300+ full computational graphs and parameter references into RAM would rapidly trigger a catastrophic Out-Of-Memory (OOM) fatal error. You must strip the historical autograd memory to save just the plain numpy matrices.}
\vspace{0.75cm}
...
\textcolor{blue}{\textbf{Solution:} When using an excessively huge learning rate, the network's gradient updates "overshoot" the minimum loss landscape violently instead of steadily descending it. The student would see the UMAP clusters thrashing or teleporting wildly across the visualizer grid at random. Instead of cleanly forming tight conceptual islands that smoothly morph from chaos into steady structure, the geometry would oscillate and fail to capture any semantic distinctions cleanly.}
\vspace{0.75cm}
...
\textcolor{blue}{\textbf{Solution:} [Answers will vary but should describe heavily overlapping lexicons]. For example: \textit{``Astronomy''} and \textit{``Astrophysics''}. Because the actual words used in both domains (stars, galaxy, gravity, telescope, observation) are extremely similar, the TF-IDF inputs are nearly identical. The neural network will struggle to separate these two classes significantly without a much deeper architecture, leading to messy, blurred boundary manifolds in the UMAP plot where points merge.}
\vspace{0.75cm}
